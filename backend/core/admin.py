...
class ProjectAdmin(admin.ModelAdmin):
    list_display = ('name', 'organization', 'status', 'provider', 'owner', 'workspace_count', 'created_at')
    ...
    fieldsets = (
        ...
        ('Configuration', {
            'fields': ('status', 'provider', 'provider_config')
        }),
        ('Ownership', {
            'fields': ('owner',)
        }),
...
class WorkspaceAdmin(admin.ModelAdmin):
    list_display = ('name', 'project', 'environment', 'status_badge', 'terraform_version', 'state_version', 'last_applied_at')
    list_filter = ('status', 'environment', 'terraform_version', 'project__organization')
    search_fields = ('name', 'description', 'project__name')
    readonly_fields = ('id', 'created_at', 'updated_at', 'state_version', 'last_applied_at')
    ...
    fieldsets = (
        ...
        ('Status', {
            'fields': ('status', 'state_version', 'last_applied_at')
        }),
        ('Variables', {
            'fields': ('variables', 'sensitive_variables'),
            'classes': ('collapse',)
        }),
        ('Metadata', {
            'fields': ('id', 'created_at', 'updated_at'),
...
class InfrastructureStateAdmin(admin.ModelAdmin):
    list_display = ('workspace', 'version', 'resource_count', 'created_by', 'created_at')
    list_filter = ('workspace__project__organization', 'workspace__project', 'created_at')
...
class DeploymentAdmin(admin.ModelAdmin):
    list_display = ('workspace', 'deployment_type', 'status_badge', 'triggered_by', 'duration', 'created_at')
    ...
    fieldsets = (
        (None, {
            'fields': ('workspace', 'deployment_type', 'status')
        }),
        ('Execution', {
            'fields': ('triggered_by', 'started_at', 'completed_at')
        }),
        ('Output', {
        ...
        ('Errors', {
            'fields': ('error_message', 'can_retry'),
            'classes': ('collapse',)
        }),
        ('Agent', {
...
class AgentSessionAdmin(admin.ModelAdmin):
    list_display = ('short_id', 'task_preview', 'status_badge', 'user', 'agents_used_display', 'token_cost', 'created_at')
    list_filter = ('status', 'workflow_type', 'project', 'created_at')
    search_fields = ('id', 'task_description', 'user__username')
    readonly_fields = ('id', 'created_at', 'updated_at', 'started_at', 'completed_at', 'llm_tokens_used', 'estimated_cost')
    raw_id_fields = ('project', 'user')
    inlines = [ChatMessageInline]
    
    fieldsets = (
        (None, {
            'fields': ('task_description', 'workflow_type', 'status')
        }),
        ('Context', {
            'fields': ('project', 'user')
        }),
        ...
        ('Cost', {
            'fields': ('llm_tokens_used', 'estimated_cost')
        }),
        ('Metadata', {
            'fields': ('id', 'created_at', 'updated_at'),
            'classes': ('collapse',)
...
class ChatMessageAdmin(admin.ModelAdmin):
    list_display = ('short_id', 'session', 'role_badge', 'content_preview', 'agent_name', 'created_at')
